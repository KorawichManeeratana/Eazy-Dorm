...
router.get('/:id', async (req, res) => {
    const { id } = req.params;
    const sql1 = `SELECT dorm_id, name, detail, address, rent, rating, phone_contact, electric_pay, water_pay, dorm_pic FROM Dormitory WHERE dorm_id = ?;`
    ...
    try {
        ...
        res.render('dorminfo', { dorm: dorm[0][0], comment: comment[0] });
